...
function getClassListFromValue(value) {
  ...
  if (typeof classes === "string") {
    classList = classes.trim().split(/\s+/).filter((k2) => k2);
  }
  return classList;
}
...
var b = ((g2) => typeof __require < "u" ? __require : typeof Proxy < "u" ? new Proxy(g2, { get: (t, e) => (typeof __require < "u" ? __require : t)[e] }) : g2)(function(g2) {
  if (typeof __require < "u") return __require.apply(this, arguments);
  throw Error('Dynamic require of "' + g2 + '" is not supported');
});
var L = (g2, t) => () => (g2 && (t = g2(g2 = 0)), t);
var F = (g2, t) => {
  for (var e in t) x(g2, e, { get: t[e], enumerable: true });
};
var W = (g2, t, e, s) => {
  if (t && typeof t == "object" || typeof t == "function") for (let n of U(t)) !I.call(g2, n) && n !== e && x(g2, n, { get: () => t[n], enumerable: !(s = J(t, n)) || s.enumerable });
  return g2;
};
var z = (g2) => W(x({}, "__esModule", { value: true }), g2);
var k;
var T;
var $;
var D;
var B;
var A = L(() => {
  k = class g2 extends Error {
    constructor(t) {
      super(t), this.name = "TokiForgeError", Object.setPrototypeOf(this, g2.prototype);
    }
  }, T = class g2 extends k {
    constructor(e, s) {
      super(e);
      this.filePath = s;
      this.name = "TokenParseError", Object.setPrototypeOf(this, g2.prototype);
    }
  }, $ = class g2 extends k {
    constructor(e, s = []) {
      super(e);
      this.errors = s;
      this.name = "TokenValidationError", Object.setPrototypeOf(this, g2.prototype);
    }
  }, D = class g2 extends k {
    constructor(e, s = []) {
      super(`Theme "${e}" not found. Available themes: ${s.join(", ")}`);
      this.availableThemes = s;
      this.name = "ThemeNotFoundError", Object.setPrototypeOf(this, g2.prototype);
    }
  }, B = class g2 extends k {
    constructor(e, s) {
      super(e);
      this.color = s;
      this.name = "ColorConversionError", Object.setPrototypeOf(this, g2.prototype);
    }
  };
});
var H = {};
...
var w = L(() => {
  ...
  d = class {
    static parse(t, e = {}) {
      if (typeof window < "u") throw new Error("TokenParser.parse() is not available in browser environments. Use TokenParser.parseJSON() instead.");
      try {
        let s = b("fs"), n = b("path"), r = b("yaml"), i = s.readFileSync(t, "utf-8"), o = n.extname(t).toLowerCase(), a;
        if (o === ".json") a = JSON.parse(i);
        else if (o === ".yaml" || o === ".yml") a = r.parse(i);
        else throw new Error(`Unsupported file format: ${o}. Supported formats: .json, .yaml, .yml`);
        return e.validate !== false && this.validate(a), e.expandReferences !== false && (a = this.expandReferences(a)), a;
      } catch (s) {
        throw new T(`Failed to parse token file: ${s instanceof Error ? s.message : "Unknown error"}`, t);
      }
    }
    static parseJSON(t, e = {}) {
      try {
        let s = JSON.parse(t);
        return e.validate !== false && this.validate(s), e.expandReferences !== false ? this.expandReferences(s) : s;
      } catch (s) {
        throw new T(`Failed to parse JSON: ${s instanceof Error ? s.message : "Unknown error"}`);
      }
    }
    static validate(t) {
      if (!t || typeof t != "object") throw new Error("Tokens must be an object");
      let e = [];
      if (this.validateTokens(t, "", e), e.length > 0) throw new $("Token validation failed", e);
    }
    static validateTokens(t, e, s) {
      for (let [n, r] of Object.entries(t)) {
        let i = e ? `${e}.${n}` : n;
        if (r == null) {
          s.push(`Token at "${i}" is null or undefined`);
          continue;
        }
        if (typeof r == "object" && !Array.isArray(r)) if ("value" in r || "$value" in r || "$alias" in r) {
          let o = r;
          if ("$alias" in o && o.$alias) typeof o.$alias != "string" && s.push(`Token alias at "${i}" must be a string`);
          else if ("value" in o || "$value" in o) {
            let a = o.value ?? o.$value;
            a == null && s.push(`Token value at "${i}" is missing`);
          } else s.push(`Token at "${i}" must have a value or alias`);
        } else this.validateTokens(r, i, s);
      }
    }
    static extractSemanticTokens(t) {
      let e = [];
      return this.extractSemanticTokensRecursive(t, "", e), e;
    }
    static extractSemanticTokensRecursive(t, e, s) {
      for (let [n, r] of Object.entries(t)) {
        let i = e ? `${e}.${n}` : n;
        if (r && typeof r == "object" && !Array.isArray(r)) if ("value" in r || "$value" in r || "$alias" in r) {
          let o = r;
          (o.semantic?.category === "semantic" || o.semantic?.category === "component") && s.push({ path: i, token: o }), o.$alias && s.push({ path: i, token: o });
        } else this.extractSemanticTokensRecursive(r, i, s);
      }
    }
    static validateAliases(t) {
      let e = [], s = /* @__PURE__ */ new Map();
      this.collectAliases(t, "", s);
      for (let [n, r] of s.entries()) this.resolveAlias(t, r) || e.push(`Alias "${r}" referenced at "${n}" not found`);
      return { valid: e.length === 0, errors: e };
    }
    static expandReferences(t) {
      let e = JSON.parse(JSON.stringify(t));
      return this.expandReferencesRecursive(e, t), e;
    }
    static expandReferencesRecursive(t, e) {
      for (let [s, n] of Object.entries(t)) if (n && typeof n == "object" && !Array.isArray(n)) if ("$alias" in n && n.$alias) {
        let r = this.resolveAlias(e, n.$alias);
        r !== null && (t[s] = __spreadProps(__spreadValues({}, n), { value: r, $alias: void 0 }));
      } else this.expandReferencesRecursive(n, e);
    }
    static collectAliases(t, e, s) {
      for (let [n, r] of Object.entries(t)) {
        let i = e ? `${e}.${n}` : n;
        r && typeof r == "object" && !Array.isArray(r) && ("$alias" in r && r.$alias ? s.set(i, r.$alias) : this.collectAliases(r, i, s));
      }
    }
    static resolveAlias(t, e) {
      let s = e.replace(/^{|}$/g, "").split("."), n = t;
      for (let r of s) if (n && typeof n == "object" && !Array.isArray(n) && r in n) n = n[r];
      else return null;
      if (n && typeof n == "object" && !Array.isArray(n)) {
        let r = n, i = r.value ?? r.$value;
        if (i != null && (typeof i == "string" || typeof i == "number")) return i;
      }
      return null;
    }
  };
...
var V = class {
  static compare(t, e, s = {}) {
    let { ignorePaths: n = [], tolerance: r = 0, strict: i = false } = s, o = this.flattenTokens(t), a = this.flattenTokens(e), l = { added: [], removed: [], changed: [], unchanged: [] }, c = /* @__PURE__ */ new Set([...Object.keys(o), ...Object.keys(a)]);
    for (let u2 of c) {
      if (n.some((m) => u2.startsWith(m))) continue;
      let h = o[u2], f = a[u2];
      h === void 0 ? l.added.push({ path: u2, value: f }) : f === void 0 ? l.removed.push({ path: u2, value: h }) : this.valuesMatch(h, f, r, i) ? l.unchanged.push({ path: u2, value: h }) : l.changed.push({ path: u2, figma: h, code: f });
    }
    return l;
  }
  static generateReport(t) {
    let e = [];
    e.push("Figma ↔ Code Token Diff Report"), e.push("=".repeat(50)), e.push(""), t.added.length > 0 && (e.push(`Added (${t.added.length}):`), t.added.forEach(({ path: n, value: r }) => {
      e.push(`  + ${n}: ${JSON.stringify(r)}`);
    }), e.push("")), t.removed.length > 0 && (e.push(`Removed (${t.removed.length}):`), t.removed.forEach(({ path: n, value: r }) => {
      e.push(`  - ${n}: ${JSON.stringify(r)}`);
    }), e.push("")), t.changed.length > 0 && (e.push(`Changed (${t.changed.length}):`), t.changed.forEach(({ path: n, figma: r, code: i }) => {
      e.push(`  ~ ${n}`), e.push(`    Figma: ${JSON.stringify(r)}`), e.push(`    Code:  ${JSON.stringify(i)}`);
    }), e.push("")), t.unchanged.length > 0 && (e.push(`Unchanged (${t.unchanged.length})`), e.push(""));
    let s = t.added.length + t.removed.length + t.changed.length;
    return s === 0 ? e.push("✅ No differences found!") : e.push(`⚠️  Found ${s} difference(s)`), e.join(`
`);
  }
  static hasMismatches(t) {
    return t.added.length > 0 || t.removed.length > 0 || t.changed.length > 0;
  }
  static exportJSON(t, e) {
    if (typeof window < "u") throw new Error("exportJSON is not available in browser environments. Use the CLI package instead.");
    try {
      b("fs").writeFileSync(e, JSON.stringify(t, null, 2));
    } catch (s) {
      throw new Error(`Failed to export JSON: ${s instanceof Error ? s.message : "Unknown error"}`);
    }
  }
  static flattenTokens(t, e = "", s = {}) {
    for (let n in t) {
      let r = t[n], i = e ? `${e}.${n}` : n;
      if (r && typeof r == "object" && !Array.isArray(r)) if ("value" in r || "$value" in r || "$alias" in r) {
        let o = "value" in r ? r.value : "$value" in r ? r.$value : null;
        s[i] = this.normalizeValue(o);
      } else this.flattenTokens(r, i, s);
    }
    return s;
  }
  static valuesMatch(t, e, s, n) {
    if (n) return t === e;
    let r = this.normalizeValue(t), i = this.normalizeValue(e);
    if (this.isColor(r) && this.isColor(i)) return this.colorsMatch(r, i, s);
    if (this.isNumeric(r) && this.isNumeric(i)) {
      let o = parseFloat(String(r)), a = parseFloat(String(i));
      return Math.abs(o - a) <= s;
    }
    return r === i;
  }
  static normalizeValue(t) {
  ...
  static isNumeric(t) {
    ...
    if (typeof t == "string") {
      let e = t.trim();
      return e.endsWith("px") || e.endsWith("rem") || e.endsWith("em") ? !isNaN(parseFloat(e)) : !isNaN(parseFloat(e)) && !isNaN(Number(e));
    }
    return false;
  }
  static isColor(t) {
    ...
    let e = t.trim();
    return e.startsWith("#") || e.startsWith("rgb") || e.startsWith("hsl") || e.startsWith("rgba") || e.startsWith("hsla");
  }
  static colorsMatch(t, e, s) {
    if (s === 0) return t.toLowerCase() === e.toLowerCase();
    let n = this.hexToRgb(this.toHex(t)), r = this.hexToRgb(this.toHex(e));
    return !n || !r ? t.toLowerCase() === e.toLowerCase() : Math.sqrt(Math.pow(n.r - r.r, 2) + Math.pow(n.g - r.g, 2) + Math.pow(n.b - r.b, 2)) <= s;
  }
  static toHex(t) {
    ...
    if (t.startsWith("rgb")) {
      let e = t.match(/\d+/g);
      if (e && e.length >= 3) {
        let s = parseInt(e[0]), n = parseInt(e[1]), r = parseInt(e[2]);
        return `#${[s, n, r].map((i) => i.toString(16).padStart(2, "0")).join("")}`;
      }
    }
    return t;
  }
  static hexToRgb(t) {
    ...
    t = t.replace(e, (n, r, i, o) => r + r + i + i + o + o);
...
var C = class {
  constructor(t) {
    ...
    this.isBrowser = typeof window < "u";
    if (!t || !t.themes || t.themes.length === 0) throw new Error("ThemeConfig must have at least one theme");
    if (this.config = t, this.currentTheme = t.defaultTheme || t.themes[0]?.name || "default", !t.themes.some((s) => s.name === this.currentTheme)) throw new Error(`Default theme "${this.currentTheme}" not found in themes`);
  }
  init(t = ":root", e = "hf") {
    if (!this.isBrowser) return;
    let s = this.config.themes.find((n) => n.name === this.currentTheme);
    if (!s) throw new Error(`Theme "${this.currentTheme}" not found`);
    this.injectCSS(s.tokens, t, e);
  }
  applyTheme(t, e = ":root", s = "hf") {
    let n = this.config.themes.find((r) => r.name === t);
    if (!n) throw new D(t, this.getAvailableThemes());
    this.currentTheme = t, this.isBrowser && (this.injectCSS(n.tokens, e, s), this.dispatchThemeChangeEvent(t, n.tokens));
  }
  getCurrentTheme() {
    return this.currentTheme;
  }
  getThemeTokens(t) {
    let e = t || this.currentTheme, s = this.config.themes.find((n) => n.name === e);
    return s ? s.tokens : {};
  }
  getAvailableThemes() {
    return this.config.themes.map((t) => t.name);
  }
  nextTheme() {
    let t = this.getAvailableThemes(), s = (t.indexOf(this.currentTheme) + 1) % t.length, n = t[s];
    return this.applyTheme(n), n;
  }
  destroy() {
    this.styleElement && this.styleElement.parentNode && (this.styleElement.parentNode.removeChild(this.styleElement), this.styleElement = null), this.systemThemeUnwatch && (this.systemThemeUnwatch(), this.systemThemeUnwatch = null);
  }
  ...
  watchSystemTheme(t) {
    ...
    return e.addEventListener ? e.addEventListener("change", s) : e.addListener(s), s(e), () => {
      e.removeEventListener ? e.removeEventListener("change", s) : e.removeListener(s);
    };
  }
  injectCSS(t, e, s) {
    if (!this.isBrowser) return;
    this.styleElement || (this.styleElement = document.createElement("style"), this.styleElement.id = "tokiforge-theme", document.head.appendChild(this.styleElement));
    let n = this.generateCSS(t, e, s);
    this.styleElement.textContent = n;
  }
  generateCSS(t, e, s) {
    let n = [];
    return this.flattenTokens(t, s, n), n.length === 0 ? `${e} {}` : `${e} {
  ${n.join(`
  `)}
}`;
  }
  flattenTokens(t, e, s, n = "") {
    for (let [r, i] of Object.entries(t)) {
      let o = n ? `${n}-${r}` : r;
      if (i && typeof i == "object" && !Array.isArray(i)) if ("value" in i || "$value" in i) {
        let a = i.value || i.$value;
        if (a != null) {
          let l = `--${e}-${o}`.replace(/[A-Z]/g, (c) => `-${c.toLowerCase()}`);
          s.push(`${l}: ${a};`);
        }
      } else this.flattenTokens(i, e, s, o);
    }
  }
  dispatchThemeChangeEvent(t, e) {
    if (!this.isBrowser) return;
    let s = new CustomEvent("tokiforge:theme-change", { detail: { theme: t, tokens: e } });
    window.dispatchEvent(s);
  }
};
w();
var S = class {
  static exportCSS(t, e = {}) {
    let s = e.selector || ":root", n = e.prefix || "hf", r = [];
    return this.flattenTokens(t, n, r), r.length === 0 ? `${s} {}` : `${s} {
  ${r.join(`
  `)}
}`;
  }
  static exportSCSS(t, e = {}) {
    let s = e.prefix || "hf", n = [];
    return this.flattenTokens(t, s, n, true), n.join(`
`);
  }
  static exportJS(t, e = {}) {
    let s = e.prefix || "hf", n = {};
    return this.flattenTokensToObject(t, s, n), `module.exports = ${JSON.stringify(n, null, 2)};`;
  }
  static exportTS(t, e = {}) {
    let s = e.prefix || "hf", n = {};
    return this.flattenTokensToObject(t, s, n), `export const tokens = ${JSON.stringify(n, null, 2)} as const;`;
  }
  static exportJSON(t) {
  ...
  static export(t, e) {
    ...
    switch (s) {
      case "css":
        return this.exportCSS(t, e);
      case "scss":
        return this.exportSCSS(t, e);
      case "js":
        return this.exportJS(t, e);
      case "ts":
        return this.exportTS(t, e);
      case "json":
        return this.exportJSON(t);
      default:
        throw new Error(`Unsupported export format: ${s}`);
    }
  }
  static flattenTokens(t, e, s, n = false, r = "") {
    for (let [i, o] of Object.entries(t)) {
      let a = r ? `${r}-${i}` : i;
      if (o && typeof o == "object" && !Array.isArray(o)) if ("value" in o || "$value" in o) {
        let l = o.value ?? o.$value;
        if (l != null) {
          let c = `--${e}-${a}`.replace(/[A-Z]/g, (u2) => `-${u2.toLowerCase()}`);
          n ? s.push(`$${e}-${a.replace(/-/g, "-")}: ${l};`) : s.push(`${c}: ${l};`);
        }
      } else this.flattenTokens(o, e, s, n, a);
    }
  }
  static flattenTokensToObject(t, e, s, n = "") {
    for (let [r, i] of Object.entries(t)) {
      let o = n ? `${n}.${r}` : r;
      if (i && typeof i == "object" && !Array.isArray(i)) if ("value" in i || "$value" in i) {
        let a = i.value ?? i.$value;
      ...
      } else this.flattenTokensToObject(i, e, s, o);
    }
  }
};
var p = class {
  static hexToRgb(t) {
    if (!t.startsWith("#")) return null;
    let e = t.slice(1);
    if (e.length !== 3 && e.length !== 6) return null;
    let s = e.length === 3 ? e.split("").map((o) => o + o).join("") : e, n = parseInt(s.slice(0, 2), 16), r = parseInt(s.slice(2, 4), 16), i = parseInt(s.slice(4, 6), 16);
    return isNaN(n) || isNaN(r) || isNaN(i) ? null : { r: n, g: r, b: i };
  }
  static rgbToHex(t, e, s) {
    let n = (r) => Math.max(0, Math.min(255, Math.round(r)));
    return `#${[t, e, s].map(n).map((r) => r.toString(16).padStart(2, "0")).join("")}`;
  }
  static rgbToHsl(t, e, s) {
    t /= 255, e /= 255, s /= 255;
    let n = Math.max(t, e, s), r = Math.min(t, e, s), i = 0, o = 0, a = (n + r) / 2;
    if (n !== r) {
      let l = n - r;
    ...
    return { h: Math.round(i * 360), s: Math.round(o * 100), l: Math.round(a * 100) };
  }
  static hslToRgb(t, e, s) {
    ...
    if (e === 0) n = r = i = s;
      ...
      let o = (c, u2, h) => (h < 0 && (h += 1), h > 1 && (h -= 1), h < 0.16666666666666666 ? c + (u2 - c) * 6 * h : h < 0.5 ? u2 : h < 0.6666666666666666 ? c + (u2 - c) * (0.6666666666666666 - h) * 6 : c), a = s < 0.5 ? s * (1 + e) : s + e - s * e, l = 2 * s - a;
      n = o(l, a, t + 1 / 3), r = o(l, a, t), i = o(l, a, t - 1 / 3);
    }
    return { r: Math.round(n * 255), g: Math.round(r * 255), b: Math.round(i * 255) };
  }
  static lighten(t, e) {
    let s = this.hexToRgb(t);
    if (!s) return t;
    let n = this.rgbToHsl(s.r, s.g, s.b);
    n.l = Math.min(100, n.l + e);
    let r = this.hslToRgb(n.h, n.s, n.l);
  ...
  static saturate(t, e) {
    ...
    if (!s) return t;
    let n = this.rgbToHsl(s.r, s.g, s.b);
    n.s = Math.min(100, n.s + e);
    let r = this.hslToRgb(n.h, n.s, n.l);
    return this.rgbToHex(r.r, r.g, r.b);
  }
  static desaturate(t, e) {
    let s = this.hexToRgb(t);
    if (!s) return t;
    let n = this.rgbToHsl(s.r, s.g, s.b);
    n.s = Math.max(0, n.s - e);
    let r = this.hslToRgb(n.h, n.s, n.l);
    return this.rgbToHex(r.r, r.g, r.b);
  }
  static generateShades(t, e = 10) {
    let s = {}, n = 100 / (e - 1);
    for (let r = 0; r < e; r++) {
      let i = r * n, o = this.hexToRgb(t);
      if (!o) continue;
      let a = this.rgbToHsl(o.r, o.g, o.b);
      a.l = i;
      let l = this.hslToRgb(a.h, a.s, a.l);
      s[`${r * 100}`] = this.rgbToHex(l.r, l.g, l.b);
    }
    return s;
  }
  static generatePalette(t, e = 5) {
    let s = this.generateShades(t, e);
    return Object.values(s);
  }
  static getContrastRatio(t, e) {
    let s = this.hexToRgb(t), n = this.hexToRgb(e);
    if (!s || !n) return 0;
    let r = this.getLuminance(s.r, s.g, s.b), i = this.getLuminance(n.r, n.g, n.b), o = Math.max(r, i), a = Math.min(r, i);
    return (o + 0.05) / (a + 0.05);
  }
  static isAccessible(t, e, s = "AA") {
    let n = this.getContrastRatio(t, e);
    return s === "AA" ? n >= 4.5 : n >= 7;
  }
  static findAccessibleColor(t, e, s = "AA") {
    let n = this.hexToRgb(t);
    if (!n) return t;
    let r = this.rgbToHsl(n.r, n.g, n.b), i = this.getContrastRatio(t, e), o = s === "AA" ? 4.5 : 7;
    if (i >= o) return t;
    let a = this.hexToRgb(e);
    if (!a) return t;
    let l = this.getLuminance(a.r, a.g, a.b), u2 = this.getLuminance(n.r, n.g, n.b) < l;
    for (; i < o && r.l > 0 && r.l < 100; ) {
      r.l += u2 ? 5 : -5;
      let f = this.hslToRgb(r.h, r.s, r.l), m = this.rgbToHex(f.r, f.g, f.b);
      i = this.getContrastRatio(m, e);
    }
    let h = this.hslToRgb(r.h, r.s, r.l);
    return this.rgbToHex(h.r, h.g, h.b);
  }
  static generateDarkTheme(t) {
    let e = {};
    for (let [s, n] of Object.entries(t)) if (typeof n == "string" && n.startsWith("#")) {
      let r = this.hexToRgb(n);
      if (r) {
        let i = this.rgbToHsl(r.r, r.g, r.b);
        i.l = 100 - i.l;
        let o = this.hslToRgb(i.h, i.s, i.l);
        e[s] = this.rgbToHex(o.r, o.g, o.b);
      } else e[s] = n;
    } else typeof n == "object" && n !== null ? e[s] = this.generateDarkTheme(n) : e[s] = n;
    return e;
  }
  static getLuminance(t, e, s) {
    let [n, r, i] = [t, e, s].map((o) => (o = o / 255, o <= 0.03928 ? o / 12.92 : Math.pow((o + 0.055) / 1.055, 2.4)));
    return 0.2126 * n + 0.7152 * r + 0.0722 * i;
  }
};
var y = class {
  static calculateContrast(t, e) {
    let s = p.getContrastRatio(t, e);
    return { contrastRatio: s, wcagAA: s >= 4.5, wcagAAA: s >= 7, motionSafe: true, colorBlindSafe: this.checkColorBlindSafe(t, e) };
  }
  static checkAccessibility(t) {
    let e = /* @__PURE__ */ new Map(), s = [];
    this.extractColorPairs(t, "", s);
    for (let n of s) {
      let r = this.calculateContrast(n.fg, n.bg);
      e.set(n.path, r);
    }
    return e;
  }
  static checkMotionPreferences() {
    return typeof window > "u" || !window.matchMedia ? true : !window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  }
  static checkColorBlindSafe(t, e) {
    let s = p.hexToRgb(t), n = p.hexToRgb(e);
    if (!s || !n) return false;
    let r = p.rgbToHsl(s.r, s.g, s.b), i = p.rgbToHsl(n.r, n.g, n.b), o = Math.abs(r.l - i.l), a = Math.abs(r.s - i.s);
    return o >= 30 || a >= 30;
  }
  static generateAccessibilityReport(t) {
    let e = this.checkAccessibility(t), s = Array.from(e.entries()).map(([i, o]) => ({ path: i, metrics: o })), n = s.filter((i) => i.metrics.wcagAA).length, r = s.length - n;
    return { total: s.length, passing: n, failing: r, results: s };
  }
  static extractColorPairs(t, e, s) {
    for (let [n, r] of Object.entries(t)) {
      let i = e ? `${e}.${n}` : n;
      if (r && typeof r == "object" && !Array.isArray(r)) if ("value" in r || "$value" in r) {
        let o = r, a = o.value ?? o.$value;
        if (typeof a == "string" && a.startsWith("#")) {
          if (i.includes("text") || i.includes("foreground") || i.includes("fg")) {
            let l = i.replace(/text|foreground|fg/gi, "background").replace(/background|bg/gi, "background"), c = this.findTokenByPath(t, l);
            c && typeof c == "string" && c.startsWith("#") && s.push({ path: i, fg: a, bg: c });
          } else if (i.includes("background") || i.includes("bg")) {
            let l = i.replace(/background|bg/gi, "text").replace(/text|foreground|fg/gi, "text"), c = this.findTokenByPath(t, l);
            c && typeof c == "string" && c.startsWith("#") && s.push({ path: i, fg: c, bg: a });
          }
        }
      } else this.extractColorPairs(r, i, s);
    }
  }
  static findTokenByPath(t, e) {
    let s = e.split("."), n = t;
    for (let r of s) if (n && typeof n == "object" && !Array.isArray(n) && r in n) n = n[r];
    else return null;
    if (n && typeof n == "object" && !Array.isArray(n)) {
...
var j = class {
  ...
  registerComponentTheme(t) {
    if (!t.name) throw new Error("Component theme must have a name");
    this.themes.set(t.name, t);
  }
  getComponentTheme(t) {
    return this.themes.get(t);
  }
  getScopedTokens(t, e) {
    let s = this.getComponentTheme(t);
    return s ? this.mergeTokens(e, s.tokens, s.scope) : {};
  }
  extractComponentTokens(t, e) {
    let s = {};
    for (let [n, r] of Object.entries(t)) if (r && typeof r == "object" && !Array.isArray(r)) {
      let i = r;
      (i.component === e || i.scope === e) && (s[n] = r);
    }
    return s;
  }
  applyComponentTheme(t, e, s) {
    let n = this.getComponentTheme(t);
    if (!n) return "";
    let r = [];
    return this.flattenTokens(n.tokens, s || "hf", r), r.length === 0 ? `${e} {}` : `${e} {
  ${r.join(`
  `)}
}`;
  }
  mergeTokens(t, e, s) {
    ...
    for (let [r, i] of Object.entries(e)) if (s) {
      let o = `${s}.${r}`;
      n[o] = i;
    } else n[r] = i;
    return n;
...
var R = class {
  ...
  register(t) {
    if (!t.name) throw new Error("Plugin must have a name");
  ...
  export(t, e, s) {
    ...
    if (!n.exporter) throw new Error(`Plugin "${e}" does not have an exporter`);
    return n.exporter(t, s);
  }
  validate(t, e) {
    let s = this.get(e);
    return s ? s.validator ? s.validator(t) : { valid: true, errors: [] } : { valid: false, errors: [`Plugin "${e}" not found`] };
  }
  format(t, e, s) {
    let n = this.get(e);
    if (!n) throw new Error(`Plugin "${e}" not found`);
    return n.formatter ? n.formatter(t, s) : t;
  }
  validateAll(t) {
    let e = /* @__PURE__ */ new Map();
    for (let [s, n] of this.plugins.entries()) n.validator ? e.set(s, n.validator(t)) : e.set(s, { valid: true, errors: [] });
    return e;
  }
  clear() {
    this.plugins.clear();
...
var Z = new R();
var O = class {
  static getResponsiveValue(t, e, s = [{ name: "sm", min: 640 }, { name: "md", min: 768 }, { name: "lg", min: 1024 }, { name: "xl", min: 1280 }, { name: "2xl", min: 1536 }]) {
    if (!t.responsive) {
      let a = t.value || t.$value;
      return typeof a == "string" || typeof a == "number" ? a : void 0;
    }
    let n = t.responsive, r = n[e];
    if (r !== void 0) return r;
    let i = n.default;
    if (i !== void 0) return i;
    let o = t.value || t.$value;
    return typeof o == "string" || typeof o == "number" ? o : void 0;
  }
  static getStateValue(t, e) {
    if (!t.states) {
      let i = t.value || t.$value;
      return typeof i == "string" || typeof i == "number" ? i : void 0;
    }
    let s = t.states[e];
    if (s !== void 0) return s;
    let n = t.states.default;
    if (n !== void 0) return n;
    let r = t.value || t.$value;
    return typeof r == "string" || typeof r == "number" ? r : void 0;
  }
  static generateResponsiveCSS(t, e = [{ name: "sm", min: 640 }, { name: "md", min: 768 }, { name: "lg", min: 1024 }, { name: "xl", min: 1280 }, { name: "2xl", min: 1536 }], s = "hf") {
    let n = [];
    for (let r of e) {
      let i = `@media (min-width: ${r.min}px)`, o = [];
      this.flattenResponsiveTokens(t, r.name, s, o), o.length > 0 && n.push(`${i} {
  :root {
    ${o.join(`
    `)}
  }
...
`);
  ...
  static generateStateCSS(t, e = "hf") {
    let s = [];
    return this.flattenStateTokens(t, e, s), s.join(`

`);
  }
  static flattenResponsiveTokens(t, e, s, n) {
    if (n) {
      this.flattenResponsiveTokensRecursive(t, e, s, n);
      return;
    }
    let r = {};
    return this.flattenResponsiveTokensToObject(t, e || "default", r), r;
  }
  static flattenStateTokens(t, e, s, n) {
    if (Array.isArray(s)) {
      this.flattenStateTokensRecursive(t, e || "hf", s);
      return;
    }
    let r = {};
    return this.flattenStateTokensToObject(t, e || "default", r), r;
  }
  static flattenResponsiveTokensRecursive(t, e, s, n, r = "") {
    for (let [i, o] of Object.entries(t)) {
      let a = r ? `${r}-${i}` : i;
      if (o && typeof o == "object" && !Array.isArray(o)) {
        let l = o;
        if (l.responsive) {
          let c = this.getResponsiveValue(l, e);
          if (c !== void 0) {
            let u2 = `--${s}-${a}`.replace(/[A-Z]/g, (h) => `-${h.toLowerCase()}`);
            n.push(`${u2}: ${c};`);
          }
        } else if ("value" in o || "$value" in o) {
          let c = l.value || l.$value;
          if (c !== void 0) {
            let u2 = `--${s}-${a}`.replace(/[A-Z]/g, (h) => `-${h.toLowerCase()}`);
            n.push(`${u2}: ${c};`);
          }
        } else this.flattenResponsiveTokensRecursive(o, e, s, n, a);
      }
    }
  }
  static flattenResponsiveTokensToObject(t, e, s, n = "") {
    for (let [r, i] of Object.entries(t)) {
      let o = n ? `${n}.${r}` : r;
      if (i && typeof i == "object" && !Array.isArray(i)) {
        let a = i;
        if (a.responsive) {
          let l = this.getResponsiveValue(a, e);
          l !== void 0 && this.setNestedValue(s, o, { value: l, type: a.type });
        } else "value" in i || "$value" in i ? s[o] = i : this.flattenResponsiveTokensToObject(i, e, s, o);
      }
    }
  }
  static flattenStateTokensRecursive(t, e, s, n = "") {
    for (let [r, i] of Object.entries(t)) {
      let o = n ? `${n}-${r}` : r;
      if (i && typeof i == "object" && !Array.isArray(i)) {
        let a = i;
        if (a.states) {
          for (let [l, c] of Object.entries(a.states)) if (l !== "default" && c !== void 0) {
            let u2 = `--${e}-${o}-${l}`.replace(/[A-Z]/g, (h) => `-${h.toLowerCase()}`);
            s.push(`.${l} {
  ${u2}: ${c};
}`);
          }
        } else if ("value" in i || "$value" in i) {
          let l = a.value || a.$value;
          if (l !== void 0) {
            let c = `--${e}-${o}`.replace(/[A-Z]/g, (u2) => `-${u2.toLowerCase()}`);
            s.push(`${c}: ${l};`);
          }
        } else this.flattenStateTokensRecursive(i, e, s, o);
      }
    }
  }
  static flattenStateTokensToObject(t, e, s, n = "") {
    for (let [r, i] of Object.entries(t)) {
      let o = n ? `${n}.${r}` : r;
      if (i && typeof i == "object" && !Array.isArray(i)) {
        let a = i;
        if (a.states) {
          let l = this.getStateValue(a, e);
          l !== void 0 && this.setNestedValue(s, o, { value: l, type: a.type });
        } else "value" in i || "$value" in i ? s[o] = i : this.flattenStateTokensToObject(i, e, s, o);
      }
    }
  }
  static setNestedValue(t, e, s) {
    let n = e.split("."), r = t;
...
var E = class {
  ...
  trackUsage(t, e) {
    ...
    s ? (s.count++, e && s.formats.add(e), s.lastUsed = /* @__PURE__ */ new Date()) : this.usage.set(t, { path: t, count: 1, formats: new Set(e ? [e] : []), lastUsed: /* @__PURE__ */ new Date() });
  }
  getUsageReport(t) {
    let e = this.getAllTokenPaths(t), s = Array.from(this.usage.keys()), n = e.filter((r) => !s.includes(r));
    return { total: e.length, used: s.length, unused: n.length, usage: Array.from(this.usage.values()).map((r) => ({ path: r.path, count: r.count, formats: Array.from(r.formats), lastUsed: r.lastUsed })), unusedTokens: n };
  }
  generateReport(t) {
    let e = this.getUsageReport(t), s = [];
    return s.push("Token Usage Report"), s.push("=".repeat(50)), s.push(`Total Tokens: ${e.total}`), s.push(`Used Tokens: ${e.used}`), s.push(`Unused Tokens: ${e.unused}`), s.push(""), e.unusedTokens.length > 0 && (s.push("Unused Tokens:"), e.unusedTokens.forEach((n) => {
      s.push(`  - ${n}`);
    }), s.push("")), e.usage.length > 0 && (s.push("Usage Statistics:"), e.usage.sort((n, r) => r.count - n.count).slice(0, 20).forEach((n) => {
      s.push(`  ${n.path}: ${n.count} uses (${n.formats.join(", ")})`);
    })), s.join(`
`);
  }
  reset() {
  ...
  getAllTokenPaths(t, e = "") {
    let s = [];
    for (let [n, r] of Object.entries(t)) {
      let i = e ? `${e}.${n}` : n;
      r && typeof r == "object" && !Array.isArray(r) && ("value" in r || "$value" in r || "$alias" in r ? s.push(i) : s.push(...this.getAllTokenPaths(r, i)));
    }
    return s;
  }
};
var v = class {
  static getDeprecatedTokens(t) {
    let e = [];
    return this.collectDeprecatedTokens(t, "", e), e;
  }
  static getVersionHistory(t) {
    let e = /* @__PURE__ */ new Map();
    return this.collectVersionHistory(t, "", e), e;
  }
  static filterDeprecated(t, e = false) {
    if (e) return t;
    let s = JSON.parse(JSON.stringify(t));
    return this.removeDeprecatedTokens(s), s;
  }
  static migrateToken(t, e, s) {
    try {
      let n = JSON.parse(JSON.stringify(t)), r = this.getTokenByPath(n, e);
      return r ? (this.setTokenByPath(n, s, r), this.removeTokenByPath(n, e), { success: true, tokens: n, message: `Successfully migrated token from "${e}" to "${s}"` }) : { success: false, tokens: n, message: `Token at path "${e}" not found` };
    } catch (n) {
      return { success: false, tokens: t, message: `Migration failed: ${n instanceof Error ? n.message : "Unknown error"}` };
    }
  }
  static validateVersion(t, e) {
    let s = [], n = [];
    if (!e) return { valid: true, errors: s, warnings: n };
    let r = this.getVersionHistory(t);
    for (let [i, o] of r.entries()) o.deprecated && n.push(`Token "${i}" is deprecated${o.replacedBy ? `, replaced by "${o.replacedBy}"` : ""}`), o.removed && s.push(`Token "${i}" has been removed`);
    return { valid: s.length === 0, errors: s, warnings: n };
  }
  static collectDeprecatedTokens(t, e, s) {
    for (let [n, r] of Object.entries(t)) {
      let i = e ? `${e}.${n}` : n;
      if (r && typeof r == "object" && !Array.isArray(r)) {
        let o = r;
        if (o.version?.deprecated || o.deprecated) {
          let a = o.version || { version: "1.0.0", deprecated: o.deprecated ? "true" : void 0 };
          s.push({ path: i, token: o, version: a });
        } else this.collectDeprecatedTokens(r, i, s);
      }
    }
  }
  static collectVersionHistory(t, e, s) {
    for (let [n, r] of Object.entries(t)) {
      let i = e ? `${e}.${n}` : n;
      if (r && typeof r == "object" && !Array.isArray(r)) {
        let o = r;
        o.version ? s.set(i, o.version) : this.collectVersionHistory(r, i, s);
      }
    }
  }
  static removeDeprecatedTokens(t) {
    for (let [e, s] of Object.entries(t)) if (s && typeof s == "object" && !Array.isArray(s)) {
      let n = s;
      n.deprecated || n.version?.deprecated ? delete t[e] : this.removeDeprecatedTokens(s);
    }
  }
  static getTokenByPath(t, e) {
    let s = e.split("."), n = t;
    for (let r of s) if (n && typeof n == "object" && !Array.isArray(n) && r in n) n = n[r];
    else return null;
    return n || null;
  }
  static setTokenByPath(t, e, s) {
    let n = e.split("."), r = t;
    for (let i = 0; i < n.length - 1; i++) {
      let o = n[i];
      o in r || (r[o] = {}), r = r[o];
  ...
  static removeTokenByPath(t, e) {
    let s = e.split("."), n = t;
    for (let r = 0; r < s.length - 1; r++) {
      let i = s[r];
      if (n && typeof n == "object" && !Array.isArray(n) && i in n) n = n[i];
      else return;
    }
...
var M = class {
  static validate(t, e = {}) {
    let { strict: s = false, checkAccessibility: n = true, checkVersioning: r = true, minAccessibilityLevel: i = "AA" } = e, o = [], a = [], l = [];
    try {
      d.validate(t), l.push({ name: "Token Validation", passed: true });
    } catch (u2) {
      let h = u2 instanceof Error ? u2.message : "Token validation failed";
      o.push(h), l.push({ name: "Token Validation", passed: false, message: h });
    }
    let c = d.validateAliases(t);
    if (c.valid ? l.push({ name: "Alias Validation", passed: true }) : (c.errors.forEach((u2) => {
      s ? o.push(u2) : a.push(u2);
    }), l.push({ name: "Alias Validation", passed: !s && c.errors.length === 0, message: `${c.errors.length} alias errors found` })), n) {
      let h = y.generateAccessibilityReport(t).results.filter((f) => i === "AA" ? !f.metrics.wcagAA : !f.metrics.wcagAAA);
      if (h.length === 0) l.push({ name: "Accessibility", passed: true });
      else {
        let f = `${h.length} tokens fail ${i} accessibility requirements`;
        s ? h.forEach((m) => {
          o.push(`Token "${m.path}" fails accessibility: contrast ratio ${m.metrics.contrastRatio.toFixed(2)}`);
        }) : a.push(f), l.push({ name: "Accessibility", passed: false, message: f });
      }
    }
    if (r) {
      let u2 = v.validateVersion(t);
      u2.errors.forEach((h) => o.push(h)), u2.warnings.forEach((h) => a.push(h)), l.push({ name: "Version Validation", passed: u2.valid, message: u2.errors.length > 0 ? `${u2.errors.length} errors, ${u2.warnings.length} warnings` : void 0 });
    }
    return { passed: o.length === 0, errors: o, warnings: a, checks: l };
  }
  static validateFile(t, e = {}) {
    try {
      let s = d.parse(t);
      return this.validate(s, e);
    } catch (s) {
      return { passed: false, errors: [s instanceof Error ? s.message : "Failed to parse token file"], warnings: [], checks: [{ name: "File Parsing", passed: false, message: "Failed to read or parse file" }] };
    }
  }
  static generateReport(t) {
    let e = [];
    return e.push("CI/CD Validation Report"), e.push("=".repeat(50)), e.push(`Status: ${t.passed ? "PASSED" : "FAILED"}`), e.push(""), t.checks.length > 0 && (e.push("Checks:"), t.checks.forEach((s) => {
      let n = s.passed ? "✓" : "✗";
      e.push(`  ${n} ${s.name}${s.message ? `: ${s.message}` : ""}`);
    }), e.push("")), t.errors.length > 0 && (e.push("Errors:"), t.errors.forEach((s) => {
      e.push(`  ✗ ${s}`);
    }), e.push("")), t.warnings.length > 0 && (e.push("Warnings:"), t.warnings.forEach((s) => {
      e.push(`  ⚠ ${s}`);
    }), e.push("")), e.join(`
`);
  }
...
var P = class {
  ...
  loadTokensFromFile(t) {
    if (typeof window < "u") throw new Error("IDESupport.loadTokensFromFile() is not available in browser environments");
    try {
      let { TokenParser: e } = (w(), z(H));
      this.tokens = e.parse(t);
    } catch (e) {
      throw new Error(`Failed to load tokens from file: ${e instanceof Error ? e.message : "Unknown error"}`);
    }
  }
  getHoverInfo(t) {
    let e = this.getTokenByPath(t);
    if (!e) return null;
    let s = e.value ?? e.$value;
    return { path: t, value: (typeof s == "string" || typeof s == "number" ? s : "") || "", type: e.type, description: e.description, deprecated: e.deprecated || e.version?.deprecated !== void 0 };
  }
  getCompletions(t) {
    let e = [], s = this.getAllTokenPaths(this.tokens);
    for (let n of s) if (!t || n.startsWith(t)) {
      let r = this.getTokenByPath(n);
      if (r) {
        let i = r.value ?? r.$value;
        e.push({ label: n, kind: "value", detail: typeof i == "string" ? i : String(i), documentation: r.description });
      }
    }
    return e;
  }
  findDefinitions(t) {
    let e = [], s = this.getAllTokenPaths(this.tokens);
    for (let n of s) if (n.includes(t) || n.toLowerCase().includes(t.toLowerCase())) {
      let r = this.getTokenByPath(n);
      if (r) {
        let i = r.value ?? r.$value, o = typeof i == "string" || typeof i == "number" ? i : "";
        e.push({ path: n, value: o || "", type: r.type });
      }
    }
    return e;
  }
  getTokenSuggestions(t) {
    let e = [], s = this.getAllTokenPaths(this.tokens), n = t.toLowerCase();
    for (let r of s) if (r.toLowerCase().includes(n)) {
      let i = this.getTokenByPath(r);
      if (i) {
        let o = i.value ?? i.$value;
        e.push({ label: r, kind: "value", detail: typeof o == "string" ? o : String(o), documentation: i.description });
      }
    }
    return e.slice(0, 10);
  }
  generateTokenDocumentation(t) {
    let e = this.getTokenByPath(t);
    if (!e) return `Token "${t}" not found`;
    let s = e.value ?? e.$value, n = [];
    return n.push(`# ${t}`), n.push(""), e.description && (n.push(e.description), n.push("")), n.push(`**Type:** ${e.type || "custom"}`), n.push(`**Value:** ${s !== void 0 ? String(s) : "N/A"}`), (e.deprecated || e.version?.deprecated) && (n.push("**Status:** Deprecated"), e.version?.replacedBy && n.push(`**Replaced by:** ${e.version.replacedBy}`)), e.version && n.push(`**Version:** ${e.version.version}`), n.join(`
`);
  }
  getTokenByPath(t) {
    let e = t.split("."), s = this.tokens;
    for (let n of e) if (s && typeof s == "object" && !Array.isArray(s) && n in s) s = s[n];
    else return null;
    if (s && typeof s == "object" && !Array.isArray(s)) {
      let n = s;
      if ("value" in n || "$value" in n || "$alias" in n) return n;
    }
    return null;
  }
  getAllTokenPaths(t, e = "") {
...
var N = class g {
  ...
  register(t) {
    if (!t.path) throw new Error("Registry entry must have a path");
    this.entries.set(t.path, __spreadProps(__spreadValues({}, t), { team: t.team || this.config.defaultTeam, version: t.version || this.config.defaultVersion }));
  }
  get(t, e) {
    let s = this.entries.get(t);
    if (s && !(e && s.team !== e)) return s;
  }
  getAll(t) {
    let e = Array.from(this.entries.values());
    return t ? e.filter((s) => s.team === t) : e;
  }
  getByVersion(t) {
    return Array.from(this.entries.values()).filter((e) => e.version === t);
  }
  getByTag(t) {
    return Array.from(this.entries.values()).filter((e) => e.tags?.includes(t));
  }
  importFromTokens(t, e, s) {
    this.flattenAndRegister(t, "", e || this.config.defaultTeam, s || this.config.defaultVersion);
  }
  exportTokens(t, e) {
    let s = t ? this.getAll(t) : this.getAll(), n = e ? s.filter((i) => i.version === e) : s, r = {};
    for (let i of n) this.setNestedValue(r, i.path, i.value);
    return r;
  }
  merge(t, e = "override") {
    let s = t.getAll();
    for (let n of s) {
      let r = this.entries.get(n.path);
      r ? e === "override" ? this.entries.set(n.path, n) : e === "merge" && this.entries.set(n.path, __spreadValues(__spreadValues({}, r), n)) : this.entries.set(n.path, n);
    }
  }
  conflictCheck(t) {
    let e = [];
    for (let [s, n] of this.entries.entries()) {
      let r = t.get(s);
      r && JSON.stringify(n.value) !== JSON.stringify(r.value) && e.push({ path: s, local: n, remote: r });
    }
    return e;
  }
  getVersionHistory(t, e) {
    return (e ? this.getAll(e) : this.getAll()).filter((n) => n.path === t).sort((n, r) => {
      let i = n.version || "0.0.0", o = r.version || "0.0.0";
      return i.localeCompare(o);
    });
  }
  listTeams() {
    let t = /* @__PURE__ */ new Set();
    for (let e of this.entries.values()) e.team && t.add(e.team);
    return Array.from(t);
  }
  serialize() {
    return JSON.stringify({ config: this.config, entries: Array.from(this.entries.entries()) }, null, 2);
  }
  static deserialize(t) {
    let e = JSON.parse(t), s = new g(e.config);
    if (e.entries) for (let [n, r] of e.entries) s.entries.set(n, r);
    return s;
  }
  flattenAndRegister(t, e, s, n) {
    for (let [r, i] of Object.entries(t)) {
      let o = e ? `${e}.${r}` : r;
      i && typeof i == "object" && !Array.isArray(i) && ("value" in i || "$value" in i ? this.register({ path: o, value: i, team: s, version: n }) : this.flattenAndRegister(i, o, s, n));
    }
  }
  setNestedValue(t, e, s) {
    let n = e.split("."), r = t;
    for (let i = 0; i < n.length - 1; i++) {
      let o = n[i];
      o in r || (r[o] = {}), r = r[o];
...
var ThemeService = class {
  ...
  tokens = this.tokensSignal.asReadonly();
  availableThemes = computed(() => {
    return this.runtime ? this.runtime.getAvailableThemes() : [];
  });
  constructor() {
  }
  init(config2, options = {}) {
    this.options = __spreadValues({}, options);
    this.runtime = new C(config2);
    const themeName = options.defaultTheme || config2.defaultTheme || config2.themes[0]?.name || "default";
    if (this.isBrowser) {
      ...
      this.themeSignal.set(themeName);
      this.tokensSignal.set(config2.themes[0]?.tokens || {});
    }
  }
  setTheme(themeName) {
  ...
  getContext() {
    ...
    return {
      ...
      generateCSS: this.options.mode === "static" ? (name) => this.generateCSS(name) : void 0
    };
  }
  ngOnDestroy() {
    if (this.runtime) {
...
function generateThemeCSS(config2, options = {}) {
  const runtime = new C(config2);
  const result = {};
  for (const theme of config2.themes) {
...
__name(generateThemeCSS, "generateThemeCSS");
function generateCombinedThemeCSS(config2, options = {}) {
  const files = generateThemeCSS(config2, options);
  return Object.values(files).join("\n\n");
}
__name(generateCombinedThemeCSS, "generateCombinedThemeCSS");
export {
  y as AccessibilityUtils,
  M as CICDValidator,
  B as ColorConversionError,
  p as ColorUtils,
  j as ComponentTheming,
  V as FigmaDiff,
  P as IDESupport,
  R as PluginManager,
  O as ResponsiveTokens,
  D as ThemeNotFoundError,
  C as ThemeRuntime,
  ThemeService,
  E as TokenAnalytics,
  S as TokenExporter,
  T as TokenParseError,
  d as TokenParser,
  N as TokenRegistry,
  $ as TokenValidationError,
  v as TokenVersioning,
  k as TokiForgeError,
  generateCombinedThemeCSS,
  generateThemeCSS,
  Z as pluginManager
};
